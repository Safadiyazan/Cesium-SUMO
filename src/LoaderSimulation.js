...
export async function LoadSimulation(viewer, data, dataSUMO1, dataSUMO2, dataSUMO3, city) {
    ...
    function ViewSetting(center, NavigationOn) {
        ...
        // Adjust pitch to keep the center point in view
        const viewaltitude = 1000; // Altitude in meters
        var initialPosition = computeNewPoint(center, 0, 0, viewaltitude);
        const viewdistanceToCenter = Math.tan(Cesium.Math.toRadians(45)) * viewaltitude; // Adjust for field of view
        const viewpitch = Math.atan2(-viewaltitude, viewdistanceToCenter); // Dynamic pitch based on distance and altitude
    ...
    async function AddCloud(centerCloud, startSim, stopSim) {
        ...
        var cloudCollection = viewer.scene.primitives.add(new CloudCollection());

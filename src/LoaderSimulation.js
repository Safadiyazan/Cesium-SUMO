...
export async function LoadSimulation(viewer, data, city) {
        ...
        data.ObjSUMO.forEach((ObjSUMO, index) => {
            if ((index >= 0) & (index < 200)) {
                ...
                for (let i = 0; i < ObjSUMO.lon.length; i += dt) {
                    // const cartographic = Cartographic.fromDegrees(ObjSUMO.lon[i], ObjSUMO.lat[i]);
                    // // var agent_height_defined = false;
                    // // if (agent_height_defined !== true) {
                    // const objectsToExclude = [];
                    // let lonlatHeight = dz0;
                    // let Checkheight;
                    // if (scene.sampleHeightSupported) {
                    //     Checkheight = scene.sampleHeight(cartographic, objectsToExclude);
                    // }
                    // if (Cesium.defined(Checkheight)) {
                    //     lonlatHeight = Checkheight;
                    //     // agent_height_defined = true;
                    // }
                    // }
                    // TODO ADD PERVIOUS HIEGHT IF CANT FIND
                    // console.log("Height of point:", lonlatHeight);
                    trajectoryPositions.push({
    ...
    async function AddAgentMotion(startSim, stopSim, timeStepInSeconds, AgentIndex, trajData, AMI, statusData, tda, taa, rs, rd, vertical, entitiesArray, positionPropertyArray) {
        ...
        // trajData.forEach((dataPoint) => {
        //     pointsCollection.add({
        //         position: Cesium.Cartesian3.fromDegrees(dataPoint.longitude, dataPoint.latitude, dataPoint.height || 0.0),
        //         pixelSize: 5,
        //         color: Cesium.Color.RED,
        //     });
        // });
        ...
        // const entitiesAhead = [];
        // const windowSize = 5;  // Number of points ahead and behind to show

        // // Initialize entities for the entire trajectory, clamped to terrain
        // trajData.forEach((dataPoint, i) => {
        //     const position = Cesium.Cartesian3.fromDegrees(dataPoint.longitude, dataPoint.latitude, dataPoint.height || 0.0);

        //     // Entity for points behind
        //     const entityBehind = viewer.entities.add({
        //         position: position,
        //         point: {
        //             pixelSize: 5,
        ...
        //     });
        //     entitiesBehind.push(entityBehind);

        //     // Entity for points ahead
        //     const entityAhead = viewer.entities.add({
        //         position: position,
        //         point: {
        //             pixelSize: 5,
        ...
        //         show: false,  // Initially hidden
        //     });
        //     entitiesAhead.push(entityAhead);
        // });

        // // Function to update the visibility of entities in the window around the current step
        // function updateTrajectoryWindow(currentStep) {
        //     // Hide all entities initially
        //     entitiesBehind.forEach(entity => entity.show = false);
        //     entitiesAhead.forEach(entity => entity.show = false);

        //     // Show entities behind the agent within the window size
        //     for (let i = Math.max(0, currentStep - windowSize); i < currentStep; i++) {
        //         entitiesBehind[i].show = true;
        //     }

        //     // Show entities ahead of the agent within the window size
        //     for (let i = currentStep + 1; i <= Math.min(trajData.length - 1, currentStep + windowSize); i++) {
        //         entitiesAhead[i].show = true;
        //     }
        // }

        // // Assume a time-based function or event updates `currentStep` as the agent moves
        // let currentStep = tda;
        // viewer.clock.onTick.addEventListener(() => {
        //     // Update window visibility based on the current step
        //     updateTrajectoryWindow(currentStep);
        //     currentStep = (currentStep + 1) % trajData.length; // Loop for demo; replace with actual logic
        // });



        // Add Aircraft Safety Radius Sphere
        const SafetySphereEntity = viewer.entities.add({
        ...
        // Load SUMO Model with optimized properties
    ...
    // // var AgentURL = "/YS_Drone_Pack.glb";
